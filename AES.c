...
//typedef unsigned int  uint32_t;   //格式化输出%u
...
void loadStateArray(uint8_t state[4][4], const uint8_t *input) { //转化为状态矩阵
    int i,j;
    for (i = 0; i < 4; ++i) {
        for (j = 0; j < 4; ++j) {
...
void storeStateArray(uint8_t *output,uint8_t state[4][4]) {   //状态矩阵转化为输出矩阵
    int i,j;
    for (i = 0; i < 4; ++i) {
        for (j = 0; j < 4; ++j) {
            *output++ = state[j][i];
...
void ByteSub(uint8_t state[4][4],const unsigned char *Table){ //字节替换
    int i,j;
    for (i = 0; i < 4; ++i) {
        for (j = 0; j < 4; ++j) {
...
void ShiftRows(uint8_t state[4][4]){ //行移位
    int i,j;
    uint8_t temp[4][4];
    for(i=0;i<4;i++){
        for(j=0;j<4;j++){
...
void InShiftRows(uint8_t state[4][4]){  //逆行移位
    int i,j;
    uint8_t temp[4][4];
    for(i=0;i<4;i++){
        for(j=0;j<4;j++){
...
void AddRoundKey(uint8_t state[4][4],uint8_t W[4][4]){  //轮密钥加法
    int i,j;
    for(i=0;i<4;i++){
        for(j=0;j<4;j++){
            state[j][i]^=W[j][i];
...
void RotateL(uint8_t* output,uint8_t* input,int len,int num){  //左循环移位
	int i;
	for(i=0;i<len;i++){
		output[i]=input[(i+num)%len];
	}
};
void Encrypt(uint8_t *output,const uint8_t *key_in,const uint8_t *input){
    int i,j,k;
    uint8_t state[4][4];
    KeyExpansion(key_in,w);
    loadStateArray(state,input);  //转化为状态矩阵
    ...
    storeStateArray(output,state);

};
void Decrypt(uint8_t *output,uint8_t *key_in, uint8_t*input){
    int i,j,k;
    uint8_t state[4][4];
    KeyExpansion(key_in,w);
    loadStateArray(state,input);  //转化为状态矩阵
...
void ECB(const uint8_t* plaintext, const uint8_t* keytext, uint8_t** ciphertext,const int length) {
	...
	*ciphertext=malloc(length+1); //动态分配内存，记住加1，需要以NULL结尾
	...
	for(i=0;i<num;i++){
		...
		Encrypt(cipher_set,keytext,plain_set);
		for(j=0;j<16;j++){
...
void ECB_decrypt(uint8_t** plaintext,const uint8_t* ciphertext,const uint8_t* keytext,const int length){
	...
	*plaintext=malloc(length+1); //动态分配内存，记住加1，需要以NULL结尾
	...
	for(i=0;i<num;i++){
		...
		Decrypt(plain_set,keytext,cipher_set);
		for(j=0;j<16;j++){
...
void CBC(const uint8_t* plaintext, const uint8_t* keytext, const uint8_t* vitext, uint8_t** ciphertext,const int length) {
	...
	*ciphertext=malloc(length+1);
	...
	for(i=0;i<num;i++){
		...
		Encrypt(cipher_set,keytext,plain_set);  //加密得密文分组一
...
void CFB(const uint8_t* plaintext, const uint8_t* keytext, const uint8_t* vitext, uint8_t** ciphertext,int length) {
	...
	*ciphertext=malloc(length+1); //动态分配内存，记住加1，需要以NULL结尾
	...
	for(i=0;i<num;i++){
        Encrypt(hex_register,keytext,hex_temp1);
...
void OFB(const uint8_t* plaintext, const uint8_t* keytext, const uint8_t* vitext, uint8_t** ciphertext,const int length) {
	...
	*ciphertext=malloc(length+1);
	...
	Table_copy(hex_temp1,vitext,16);
	for(i=0;i<num;i++){
        Encrypt(cipher_set,keytext,iv_temp);  //寄存器内容进行加密
...
void OFB_decrypt(uint8_t** plaintext,const uint8_t* ciphertext,const uint8_t* keytext,const uint8_t* vitext,const int length){
	...
	*plaintext=malloc(length+1); //动态分配内存，记住加1，需要以NULL结尾
    ...
    Table_copy(iv_temp,vitext,16);
	for(i=0;i<num;i++){
        Encrypt(plain_set,keytext,iv_temp);  //寄存器内容进行加密
